...
class pembayaranspp extends CI_Controller {
    ...
    public function index(){
        if($this->input->post('submit') || in_array("Pencarian persiswa-9", $this->session->fitur)){
            ...
            $data['tgl_terima'] = $siswa->tgl_terima;
            $where4=array(
                "nama" => "SPP",
            ...
            $data['histori'] = $this->M_spp->get_row_group($where2)->result();
            
            $data['form_action']='page/pembayaranspp';
            $data['form_action2']='page/pembayaranspp';
            $data['form_action3']='page/pembayaranspp';
            $data['kls']="";
    ...
    public function cetakspp($nisn=NULL, $tgl=NULL){
        $where2=array(
            'nisn'=>$nisn,
            "DATE_FORMAT(waktu, '%Y-%m-%d') =" => $tgl
        );

        $data['siswa'] = $this->M_siswa->get_row(array('nisn'=>$nisn))->row();
        $data['tanggal'] = $tgl;

        $data['transaksi'] = $this->M_spp->get_row_spp($where2)->result();
        $this->load->view('page/view_cetak_spp',$data);
    }
