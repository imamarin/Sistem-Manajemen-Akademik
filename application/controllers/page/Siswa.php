...
class siswa extends CI_Controller {
    function __construct(){
        ...
        $this->session->set_userdata(array('menu'=>'1','katmenu'=>'1'));
    }
    

    public function index(){
        $data['siswa'] = $this->M_siswa->get_all_join3()->result();
        $data['tahun'] = $this->M_tahunajaran->get_all()->result();
        $data['kelas'] = $this->M_kelas->get_all()->result();
        $data['form_action']='page/siswa/reset/';
        $this->template->admin('page/view_siswa',$data);
    }
    ...
    public function add(){
        $data=array(
            ...
            "masuktahunajaran"=>$this->input->post('masuktahunajaran'),
    ...
    public function simpan(){
        if($this->input->post('submit')){
            ...
            if($cek < 1){
                ...
                if($query==0){
                    $queri=$this->M_siswa->add($data);
                    if($queri==TRUE){
                        $this->session->set_flashdata('info',"Data telah disimpan");
                    }else{
                        $this->session->set_flashdata('info',"Data gagal disimpan");
                    }
                    redirect("page/siswa/add");
                }else{
                    $this->session->set_flashdata('info',"Data NISN sudah ada");
                    $this->add(); 
                }
            }
        }else{
    ...
    public function aktivasi($id=NULL,$stt=NULL){
        ...
        $queri=$this->M_siswa->update($where, $data);
        if($queri==TRUE){
            $this->session->set_flashdata('info',"Data Aktivasi telah diubah");
        }else{
            $this->session->set_flashdata('info',"Data Aktivasi gagal diubah");
        }
        redirect("page/siswa");
    }
    ...
    public function eksport2(){
        ...
        $excel->setActiveSheetIndex(0)->setCellValue('V3', "KELAS");
        ...
        foreach($siswa_data as $row){ // Ambil semua data dari hasil eksekusi $sql
          ...
          $excel->setActiveSheetIndex(0)->setCellValue('V'.$numrow,  $row->kdkelas);
