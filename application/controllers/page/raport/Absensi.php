...
class absensi extends CI_Controller {
    function __construct(){
        ...
        $this->session->set_userdata(array('menu'=>'54','katmenu'=>'12'));
    ...
    public function index(){
        ...
        $data['form_action']='page/raport/absensi/tampil';
        $data['semester'] = $this->session->semesterraport;
        $data['tahun'] = $row->tahun;
        $data['siswa'] = array();
        $data['kdkelas'] = "";
        $data['kls']="";
        $this->template->admin('page/raport/view_absensi_raport',$data);
    }
    ...
    function tampil(){
        ...
        if($this->input->post('input') || $this->input->post('submit')){
            ...
            $data['siswa'] = $this->M_siswa->get_row_join_absensi_raport($where,$this->session->semesterraport)->result();
    ...
    public function simpan2(){
        ...
        if($this->input->post('submit')=='simpan'){
            ...
            foreach ($nisn as $key => $value) {
                ...
                $query = $this->M_absensi_raport->get_row($where)->num_rows();
                if($query<1){
                    $queri=$this->M_absensi_raport->add($data); 
                    if($queri==TRUE){
                    ...
                    $queri=$this->M_absensi_raport->update($where,$data); 
                    if($queri==TRUE){
                        $this->session->set_flashdata('info',"Data berhasil disimpan");
                    }else{
                        $this->session->set_flashdata('info',"Data gagal disimpan");
