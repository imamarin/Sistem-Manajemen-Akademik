						...
						foreach ($matpel as $key => $dtm) {
							...
							for($c=2;$c>=0;$c--){
								for($b=0;$b<count($semes);$b++){
									if($c==0){
										$nilai=$this->db->query("SELECT pengetahuan, keterampilan FROM nilairaport,detailnilairaport,settahunajaran WHERE nilairaport.idnilairaport=detailnilairaport.idnilairaport AND nilairaport.idtahunajaran=settahunajaran.idtahunajaran AND nisn='$row->nisn' AND kdmatpel='$dtm[kdmatpel]' AND nilairaport.semester='$semes[$b]' AND tahun='$thn[$c]'");
									}else{
										$nilai=$db2->query("SELECT*FROM nilai WHERE nisn='$row->nisn' AND kdmatpel='$dtm[kdmatpel]' AND semester='$semes[$b]' AND tahun='$thn[$c]'");
									}
									
									if($nilai->num_rows($nilai)<0){
										?>
										<td align="center">-</td>
										<td align="center">-</td> 
										<?php
										...
										<td align="center">
										<?php 
										if($dtn['pengetahuan']>0){
										echo $dtn['pengetahuan'];
										$np=$np+$dtn['pengetahuan'];
										$tp++;
										}else{
										echo "-";
										}
										...
										<td align="center">
										<?php 
										if($dtn['keterampilan']>0){
										echo $dtn['keterampilan'];
										}else{
										echo "-";
										}
										?>
